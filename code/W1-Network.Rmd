...
# Set Up
```{r Load-Packages}
# load in libraries and install if necessary
...
name_mapping <- c(
  "Attention_Issues" = "AT",
  "Agressive_Conduct" = "AGR",
  "Sleep_Problems" = "SP",
  "Emotional_Reactivity" = "ER",
  "Social_Problems" = "SC",
  "Anxiety_or_Depression" = "AD",
  "Somatic_Complaints" = "SC")
...
symptom_glasso <- qgraph(
  ...
  legend = TRUE,
  legend.cex= 0.5,
  main = "CBCL Symptom Network (Graphical Lasso, r > 0.5)")
```
...
print(global_network_strength_W1)
```


# Hypothesis 2 and 3 Wave 1
```{r Centrality and EI}
...
gW1 <- graph_from_adjacency_matrix(edge_weights_matrix, mode = "undirected", weighted = TRUE, diag = FALSE)

#Convert edge weights to absolute values (lasso does allow for negative weight values)
E(gW1)$weight <- abs(E(gW1)$weight)
...
communityW1 <- cluster_walktrap(gW1, weights = E(gW1)$weight)
...
modularity_Q_W1 <- modularity(communityW1)
print(modularity_Q_W1)
```
