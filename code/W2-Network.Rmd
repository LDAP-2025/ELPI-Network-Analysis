...
cbcl_wide_wave2 <- cbcl_long_wave2 %>%
  filter(category != "Total_CBCL_Score") %>%  # exclude total
...
W2_Centrality <- centrality_auto(symptom_glasso2)
print(W2_Centrality)
```
Emotional regulation remains as the most central symptom (highest strength, closeness, betweenness, EI), followed by aggressive conduct. Patterns of the first wave remain, and there appears to be little flexibility/ change between the first two waves. The CBCL appears to be mainly driven by emotional regulation and aggressive conduct.
...
gW2 <- graph_from_adjacency_matrix(edge_weights_matrix2, mode = "undirected", weighted = TRUE, diag = FALSE)

#Convert edge weights to absolute values (lasso does allow for negative weight values)
E(gW2)$weight <- abs(E(gW2)$weight)
...
community2 <- cluster_walktrap(gW2, weights = E(gW2)$weight)
...
```
The network appears to evolve into a more random, less structured form. 
