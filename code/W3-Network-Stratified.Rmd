...
W1_cbcl_alcohol_use <- merge(alcohol_group, cbcl_wide_wave2, by = "folio")

# Merge group 1 or 8 with CBCL wide data
W1_cbcl_no_alcohol<- merge(no_alcohol_group, cbcl_wide_wave2, by = "folio")
```
...
symptom_data3SA <- W1_cbcl_alcohol_use  %>% select(-folio, -g9) #filtering participant ID 
...
# Step 1 Create Correlation matrix (excluding 'folio')
symptom_data3SN <- W1_cbcl_no_alcohol  %>% select(-folio, -g9) #filtering participant ID 
