...
```
## Data Formatting
```{r Wave 3 data Formating}
#Convert wave 2 data cbcl with new category labels and excluding total score to wide format 
cbcl_wide_wave3 <- cbcl_long_wave3 %>%
  filter(category != "Total_CBCL_Score") %>%  # optional: exclude total
...
symptom_data3 <- cbcl_wide_wave3 %>% select(-folio) #filtering participant ID 
symptom_corr3 <- cor(symptom_data3, use = "pairwise.complete.obs") # cor function computes the correlation between columns x and y,  "pairwise.complete.obscomputes each pairwise correlation using only the rows (observations) that have non-missing values for that pair of variables

# Step 2 Apply graphical lasso using qgraph
# This automatically estimates a regularized partial correlation network
symptom_glasso3 <- qgraph(
  symptom_corr3,
  ...
  nodeNames = colnames(symptom_corr3),
  theme = "Hollywood",
  fade = FALSE,
  label.cex = 1.1,
  legend = TRUE,
...
edge_weights_matrix3 <- getWmat(symptom_glasso3) #In resulting adjacency matrix each cell represents the edge weight correlation between variables can be positive (direct relationship ) or negative (inverse relationship) or 0 (no edge)
...
global_network_strength_W3 <- sum(abs(edge_weights_matrix3[upper.tri(edge_weights_matrix3)]))
print(global_network_strength_W3)
```
...
W3_Centrality <- centrality_auto(symptom_glasso3)
print(W3_Centrality)
```
Centrality and EI does not seem to differ between wave 2 and 3.
...
gW3 <- graph_from_adjacency_matrix(edge_weights_matrix3, mode = "undirected", weighted = TRUE, diag = FALSE)

#Convert edge weights to absolute values (lasso does allow for negative weight values)
E(gW3)$weight <- abs(E(gW3)$weight)
...
community3 <- cluster_walktrap(gW3, weights = E(gW3)$weight)
...
modularity_Q_W3 <- modularity(community3)
print(modularity_Q_W3)
```